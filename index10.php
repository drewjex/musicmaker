<html>
    ...
    <body>
        <?php
            ...
            $object = MusicAnalyzer::getChordProgPlusRoot($music_maker->song);
            echo "<br>";
            echo "<pre>";
            print_r($object->roots);
            echo "</pre>";
            ...
            $measures[] = $measure;
            foreach ($object->chords as $key => $value) {
                $roots = array();
                $roots[] = $object->roots[$key];
                if ($time > 15) {
                    $time = 0;
                    $notes = array();
                    $chords_empty = array();
                ...
                $new_note = new Note(array_merge($value, $roots), "Normal", $time, 1);
                $measures[(count($measures)-1)]->notes[] = $new_note;
                echo $time."\n";
                $time++;
            }
            ...
            foreach ($chords as $key => $value) {
                ...
                if ($value != -1) {
                    while (($chords[$key+$jump] == -1 || $chords[$key+$jump] === $chords[$key]) && $time+$jump < 16) {
                        $jump++;
                    }
                }
                $new_note = new Note(array($value), "Normal", $time, $jump);
                $measures[(count($measures)-1)]->notes[] = $new_note;
                echo $time."\n";
                $time += $jump;
            }*/
